...
│
├── screenshots/
│   └── selected final project evidence
│
└── data_corpus_final/
    ├── BC_Provincial/
    ├── Federal/
...
## Official Team-Reported Evaluation

The aggregate metrics below follow the team's **Final Report** and are treated as the official project results.

| Metric | Result |
|---|---:|
| Retrieval Hit Rate | 31 / 40 = **77.5%** |
| Generation Faithfulness | 17 / 40 = **42.5%** |
| Generation Failure / Non-Faithfulness Rate | 23 / 40 = **57.5%** |
| Refusal Adherence | 5 / 5 = **100%** |
| Estimated Context Precision | **~70%** |
| Estimated Context Recall | **~65%** |

The benchmark contains 10 cross-jurisdiction or trick questions. The team report calculated refusal adherence using the **five cases included in its refusal-score denominator**. Therefore, the 100% result should be read as **5 successful refusals out of 5 scored refusal cases**, not as a score across all 10 trick questions.

The 57.5% figure is presented here as a **generation failure or non-faithfulness rate**. It is the complement of the 42.5% faithfulness score and should not be interpreted as proof that every failed answer contained a fabricated claim.

The context precision and context recall values are reviewer estimates rather than directly computed question-level binary scores.
...
The reproduction-run files preserve the answers and retrieved sources returned by the local Evaluation Chatflow. They do not replace the official team-reported aggregate evaluation metrics above.
...
For this repository, the **Final Report's aggregate results** are the official project metrics. The Week 4 JSON and CSV files are retained as a separate reproducibility run and may show different answer-level behaviour.
...
The Week 4 evaluation materials preserve both successful and unsuccessful cases. The Final Report provides the official aggregate project metrics (77.5% retrieval hit rate, 42.5% generation faithfulness, and 100% refusal adherence across five scored refusal cases), while the current Week 4 notebook run is retained as a separate reproducibility check.
